...
async function isRetireInstalled() {
...
async function runRetire(pluginPath) {
    return new Promise((resolve) => {
            ...
            if (stdout) {
                try {
                    ...
                    if (retireOutput.data && Array.isArray(retireOutput.data)) {
                        const allVulnerabilities = retireOutput.data.flatMap((fileResult) => fileResult.results || []);
...
async function scanPluginManifests(vaultPath) {
    ...
    try {
        ...
        consola.error("Please ensure the vault path is correct and contains an .obsidian/plugins directory.");
        return [];
    }
...
async function main() {
    ...
    program
        ...
        if (!retireInstalled) {
            consola.error("Retire.js is not installed. Please install it using your preferred package manager:");
        ...
        const pluginsWithVulnerabilities = allResults.filter((r) => r.results.length > 0);
        if (pluginsWithVulnerabilities.length === 0) {
            consola.success("Scan complete. No vulnerabilities found.");
            return;
        }
        consola.warn("Scan complete. Vulnerabilities found in the following plugins:");
